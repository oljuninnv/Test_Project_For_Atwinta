  ...
  <form @submit.prevent="submitForm" class="max-w-lg mx-auto p-4 bg-white shadow-md rounded">
    <div>
      ...
      <div class="relative mt-2">
        <input type="text" v-model="searchTerm" @input="filterUsers" @focus="isDropdownOpen = true"
          @blur="closeDropdown" placeholder="Поиск пользователя..."
          class="input_text w-full border border-gray-300 rounded-md p-2" />
        <div v-if="isDropdownOpen && filteredUsers.length > 0"
          class="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
          <ul class="max-h-60 overflow-auto">
            <li v-for="user in filteredUsers" :key="user.id" @mousedown="selectUser(user)"
    ...
    <div>
      ...
      <div class="relative mt-2">
        <input type="text" v-model="positionSearchTerm" @input="filterPositions" @focus="isPositionDropdownOpen = true"
    ...
    <div class="mt-4">
      ...
      <div class="relative mt-2">
        <input type="text" v-model="departmentSearchTerm" @input="filterDepartments"
        ...
        <div v-if="isDepartmentDropdownOpen && filteredDepartments.length > 0"
          class="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
          <ul class="max-h-60 overflow-auto">
            <li v-for="department in filteredDepartments" :key="department.id" @mousedown="selectDepartment(department)"
              class="cursor-pointer hover:bg-gray-100 px-2 py-1">
              {{ department.name }}
            </li>
          </ul>
    ...
    <div class="mt-4">
      ...
      <input type="date" v-model="formData.adopted_at"
        class="input_text w-full border border-gray-300 rounded-md p-2 mt-2" />
    </div>

    <button type="submit"
      class="flex w-full justify-center rounded-md bg-red-500 px-3 py-1.5 text-sm font-semibold leading-6 text-white shadow-sm hover:bg-neutral-400 mt-5">Отправить</button>
  </form>
</template>
